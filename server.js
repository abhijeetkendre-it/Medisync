...
function generateOtpEmailHtml(otp) {
    ...
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #1e293b; max-width: 600px; padding: 20px;">
        <img src="cid:emaillogo" alt="Medisync Logo" style="max-height: 60px; margin-bottom: 25px; display: block;">
    ...
    try {
        ...
        try {
            await transporter.sendMail(mailOptions);
             ...
             console.error("Mail error (check .env EMAIL_USER/EMAIL_PASS). OTP:", otp);
        }
...
app.put('/api/users/me', async (req, res) => {
    try {
        ...
        const updates = {};
        const allowedUpdateFields = ['name', 'phone', 'gender', 'age', 'allergies', 'disability', 'diabetes', 'diseases', 'experience', 'hospital', 'specialty', 'fees', 'about'];
...
4. IMPORTANT FORMATTING: Do NOT use markdown. Do not use asterisks (*) for bolding or italics. Use simple dashes (-) for lists and use double line breaks for spacing out paragraphs. Output clean plain text.`;
        ...
        const response = await ai.models.generateContent({
            ...
            contents: `System Directive: ${systemPrompt}\n\nUser Message: ${message}`
        });
...
if (process.env.NODE_ENV !== 'production') {
    ...
    seedDefaultDoctor().catch(console.error);
}
